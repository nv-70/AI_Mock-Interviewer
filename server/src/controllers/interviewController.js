  ...
  try {
    ...
    if (mode === "resume" || mode === "mixed") {
      if (!resumeId) {
        ...
        if (
          resumeData &&
          resumeData.userId.toString() !== studentId.toString()
        ) {
    ...
    const selectedDifficulty = normalizeDifficultyValue(
      difficulty ||
        roleProfile.interviewStructures?.technical?.difficulty ||
        "full-time-fresher",
    );
    const defaultStructure =
      defaultInterviewStructures[selectedDifficulty] ||
      defaultInterviewStructures["full-time-fresher"];

    // Use enabledRounds if provided, otherwise use defaults based on difficulty
    let roundTypesToInclude;
    if (
      enabledRounds &&
      Array.isArray(enabledRounds) &&
      enabledRounds.length > 0
    ) {
      roundTypesToInclude = enabledRounds.filter((rt) =>
        allRoundTypes.includes(rt),
      );
    } else {
      // Use default rounds based on difficulty level
      roundTypesToInclude = Object.keys(defaultStructure).filter(
        (rt) => defaultStructure[rt].questionCount > 0,
      );
    }
    ...
    for (const roundType of roundTypesToInclude) {
      ...
      if (questionCounts && questionCounts[roundType]) {
        questionCount = Math.max(
          1,
          Math.min(20, parseInt(questionCounts[roundType])),
        );
      } else if (roleProfile.interviewStructures[roundType]?.questionCount) {
        questionCount =
          roleProfile.interviewStructures[roundType].questionCount;
      ...
      if (roundType === "technical") {
        ...
        roundDifficulty = normalizeDifficultyValue(
          roleProfile.interviewStructures[roundType].difficulty,
          selectedDifficulty,
        );
      }
    ...
    for (let i = 0; i < rounds.length; i++) {
      ...
      try {
        const roundDifficulty =
          round.customDifficulty || structure?.difficulty || selectedDifficulty;
        ...
        const context = {
          ...
          userProfile,
          questionCount:
            round.customQuestionCount || structure?.questionCount || 5,
        ...
        console.error(
          `Error generating questions for ${round.roundType}:`,
          error,
        );
...
const submitAnswer = async (req, res, next) => {
  try {
    ...
    const expectedRoundType =
      questionIdParts.length > 1 ? questionIdParts[0] : null;
    ...
    const questionExistsInRound = targetRound.questions.some(
      (q) => q.id === questionId,
    );
    ...
    try {
      ...
      evaluation = {
        score: 0,
        feedbackText:
          "Evaluation temporarily unavailable. Your answer has been saved.",
        strengths: [],
        weaknesses: [],
        improvementTips: [],
      };
...
const completeInterview = async (req, res, next) => {
  try {
    ...
    const forceComplete =
      req.method === "GET" ? true : req.body?.forceComplete || false;
    const userId = req.user._id;

    const session =
      await InterviewSession.findById(sessionId).populate("roleProfileId");
...
const buildSessionSummary = (session) => {
  ...
  averageTimePerQuestion =
    answeredCount > 0 ? totalTimeSpentSeconds / answeredCount : 0;
  const totalInterviewTimeSeconds = Math.floor(
  ...
  const estimatedTimeSeconds = rounds.reduce((total, round) => {
    return (
      total +
      round.questions.reduce((roundTotal, q) => {
...
const calculateAndReturnSummary = async (
  session,
  res,
  forceComplete = false,
) => {
    ...
    const formattedSessions = sessions.map((session) => {
      ...
      rounds.forEach((round) => {
        round.answers.forEach((answer) => {
          if (
            answer.evaluation &&
            typeof answer.evaluation.score === "number"
          ) {
            totalScore += answer.evaluation.score;
            scoreEntries++;
          }
          ...
          const submittedAt = answer.submittedAt
            ? new Date(answer.submittedAt)
            : null;
          if (
            answer.evaluation?.feedbackText &&
            (!latestFeedbackDate ||
              (submittedAt && submittedAt > latestFeedbackDate))
          ) {
      ...
      const averageScore =
        scoreEntries > 0
          ? Math.round((totalScore / scoreEntries) * 10) / 10
          : null;
